def show_eda(df):
    ...
    sns.histplot(df['Price'], kde=True, ax=ax, color="mediumseagreen")
    ...
    st.subheader("📊 Pie Charts of Price by Category")
    pie_features = ['Company', 'TypeName', 'OS']
    cols = st.columns(3)
    for i, feature in enumerate(pie_features):
        with cols[i]:
            fig, ax = plt.subplots()
            grouped = df.groupby(feature)['Price'].sum()
            ax.pie(grouped, labels=grouped.index, autopct='%1.1f%%', startangle=140)
            ax.set_title(f"Price Share by {feature}")
            st.pyplot(fig)
